    ...
    while (i < count) {
      ...
      switch (type) {
        ...
        case 4:
          ...
          break;
        default:
          RCTLogError(@"Invalid CGPath type %lld at element %lld of %@", (long long)type, (long long)i, arr);
          CGPathRelease(path);
          return NULL;
      }
    }
...
+ (ARTBrush *)ARTBrush:(id)json
{
  ...
  switch (type) {
    ...
    default:
      RCTLogError(@"Unknown brush type: %lld", (long long)type);
      return nil;
  }
}
...
+ (CGPoint)CGPoint:(id)json offset:(NSUInteger)offset
{
  ...
  if (arr.count < offset + 2) {
    RCTLogError(@"Too few elements in array (expected at least %lld): %@", 2 + (long long)offset, arr);
    return CGPointZero;
  }
  return (CGPoint){
...
+ (CGRect)CGRect:(id)json offset:(NSUInteger)offset
{
  ...
  if (arr.count < offset + 4) {
    RCTLogError(@"Too few elements in array (expected at least %lld): %@", 4 + (long long)offset, arr);
    return CGRectZero;
  }
  return (CGRect){
...
+ (CGGradientRef)CGGradient:(id)json offset:(NSUInteger)offset
{
  NSArray *arr = [self NSArray:json];
  if (arr.count < offset) {
    RCTLogError(@"Too few elements in array (expected at least %lld): %@", (long long)offset, arr);
    return NULL;
  }
